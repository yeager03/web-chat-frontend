...
// mui components
import { TextField, InputAdornment, CircularProgress, Typography, Box } from "@mui/material";
...
const DialoguesList: FC<DialoguesListProps> = (props): ReactElement => {
	...
	const empty =
		status === "success" && !dialogues.length ? <Typography variant="h3">Ничего не найдено 😔</Typography> : null;
	const sortedDialogues =
		status === "success" && dialogues.length
			? orderBy(dialogues, ["createdAt"], ["desc"]).map((dialogue) => (
					<Dialogue key={dialogue["_id"]} currentDialogueId={currentDialogueId} {...dialogue} />
			  ))
			: null;
